...
def generate_2d_mask(H=16, W=8, left=0, top=0, width=8, height=8, part=-1, cls_label=True, device='cuda'):
    ...
    mask[t: h + 1, l: w + 1] = 1  # 随机采样的值
    mask = mask.flatten(0)  # 将其展开到一维
...
class part_Attention_ViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        self.part_token1 = nn.Parameter(torch.zeros(1, 1, embed_dim))  # 局部特征
    ...
    def attn_mask_generate(self, N=132, H=16, W=8, device='cuda'):
        ...
        mask[1: 4, 0] = 0
        mask_ = (mask @ mask.t()).bool()  # pass
        mask_ |= generate_2d_mask(H, W, 0, 0, W, H / 2, 1, False, device).bool()  # 按位运算符
    ...
    def forward_features(self, x):
        ...
        x = torch.cat((cls_tokens, part_token1, part_token2, part_token3, x), dim=1)  # 多个token拼接在一起

        x = x + self.pos_embed

        x = self.pos_drop(x)
        layerwise_tokens = []  # 每层得特征拿出来

        mask = torch.ones([B, 1, self.num_patches, self.num_patches], device=x.device.type)
        # if self.training:
        # mask[:, 0] = self.mask
        # for i in range(B):
        mask[:, 0] = self.attn_mask_generate(self.num_patches, self.patch_embed.num_y, self.patch_embed.num_x,
                                             x.device.type)
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                if 'distilled' in model_path:
                    ...
                    print('Resized position embedding from size:{} to size: {} with height:{} width: {}'.format(
                        v_old.shape, v.shape, self.patch_embed.num_y, self.patch_embed.num_x))
                else:
                    v = resize_pos_embed_part_vit(v, self.pos_embed, self.patch_embed.num_y, self.patch_embed.num_x)
            elif 'cls_token' in k:
...
def part_attention_vit_large(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1,
                             **kwargs):
    model = part_Attention_ViT(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4,
        qkv_bias=True, \
...
def part_attention_vit_base(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1,
                            **kwargs):
    model = part_Attention_ViT(
...
def part_attention_vit_base_p32(img_size=(256, 128), stride_size=32, drop_rate=0.0, attn_drop_rate=0.0,
                                drop_path_rate=0.1, **kwargs):
    model = part_Attention_ViT(
        img_size=img_size, patch_size=32, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True, \
        drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
def part_attention_vit_small(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1,
                             **kwargs):
...
def part_attention_deit_small(img_size=(256, 128), stride_size=16, drop_path_rate=0.0, drop_rate=0.0,
...
def part_attention_deit_tiny(img_size=(256, 128), stride_size=16, drop_path_rate=0.0, drop_rate=0.0, attn_drop_rate=0.0,
                             **kwargs):
    model = part_Attention_ViT(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4,
        qkv_bias=True,
        drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
